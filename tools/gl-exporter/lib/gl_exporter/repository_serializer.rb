class GlExporter
  ...
  class RepositorySerializer < BaseSerializer
    ...
    def private?
      return gl_model["visibility_level"].zero? if Gitlab.api_v3?

      gl_model.fetch("visibility") == "private"
    end
    ...
    def git_url
      "tarball://root/repositories/#{gl_model["path_with_namespace"]}.git"
    end

    def wiki_url
      "tarball://root/repositories/#{gl_model["path_with_namespace"]}.wiki.git"
    end
  end
end
