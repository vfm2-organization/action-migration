...
class Gitlab
  class << self
    ...
    # Get the commits for a given merge request. Results are not paginated
    #
    # @param [#to_s] project_id the id of the merge request's project
    # @param [#to_s] merge_request_id the id of the merge request
    # @return [Array<Hash>]
    def merge_request_commits(project_id, merge_request_id)
      get("projects/#{project_id}/merge_requests/#{merge_request_id}/commits")
    end
    ...
    # Get the team members for a given project. Results are not paginated
    #
